...
const Cube = (props) => {
    ...
    const roll = () => {        
        ...
        refViewFront.setNativeProps({style: {transform: [{perspective: 1000}, {matrix: matrix}]}});
        
        matrix = rotateXY(dx + 180, dy);
        transformOrigin(matrix, origin);
        refViewBack.setNativeProps({style: {transform: [{perspective: 1000}, {matrix: matrix}]}});
    
        matrix = rotateXY(dx + 90, dy);
        transformOrigin(matrix, origin);
        refViewRight.setNativeProps({style: {transform: [{perspective: 1000}, {matrix: matrix}]}});
    
        matrix = rotateXY(dx - 90, dy);
        transformOrigin(matrix, origin);
        refViewLeft.setNativeProps({style: {transform: [{perspective: 1000}, {matrix: matrix}]}});
    
        matrix = rotateXZ(dx, dy - 90);
        transformOrigin(matrix, origin);
        refViewTop.setNativeProps({style: {transform: [{perspective: 1000}, {matrix: matrix}]}});
    
        matrix = rotateXZ(-dx, dy + 90);
        transformOrigin(matrix, origin);
        refViewBottom.setNativeProps({style: {transform: [{perspective: 1000}, {matrix: matrix}]}});
      ...
      const renderFront = (color) => {
        return (
          <View
            ...
            style={[styles.rectangle, styles.front, (color) ? {backgroundColor: color} : null]}
            {...panResponder.panHandlers}
          />
        )
      }
      ...
      const renderBack = (color) => {
        return (
          <View
            ...
            style={[styles.rectangle, styles.back, (color) ? {backgroundColor: color} : null]}
            {...panResponder.panHandlers}
          />
        )
      }
